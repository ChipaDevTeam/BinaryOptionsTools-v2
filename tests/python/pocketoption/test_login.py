...
class TestLoginInternalDetails:
    ...
    def test_login_playwright_internal_remember_checkbox_fail(self, mock_sync_pw):
        ...
        mock_pw = _make_playwright_mock(session=FAKE_SESSION)()
        mock_pw.chromium.launch.return_value.new_context.return_value.new_page.return_value.check.side_effect = Exception("checkbox not found")
    ...
    def test_login_captcha_solver_success_capsolver(self, mock_session_class):
        ...
        with patch("BinaryOptionsToolsV2.pocketoption.tools.login._solve_via_capsolver", return_value="mock_token") as mock_solve:
            session = _login_captcha_solver("u@e.com", "p", api_key="k", service="capsolver", timeout=10)
            assert session == FAKE_SESSION
    ...
    def test_login_captcha_solver_success_2captcha(self, mock_session_class):
        ...
        mock_sess.cookies.get.return_value = FAKE_SESSION
        from BinaryOptionsToolsV2.pocketoption.tools.login import _login_captcha_solver
        with patch("BinaryOptionsToolsV2.pocketoption.tools.login._solve_via_2captcha", return_value="mock_token") as mock_solve:
            session = _login_captcha_solver("u@e.com", "p", api_key="k", service="2captcha", timeout=10)
            assert session == FAKE_SESSION
            mock_solve.assert_called_once_with("k", timeout=10)
    ...
    def test_login_captcha_solver_no_session_cookie_generic(self, mock_session_class):
        ...
        from BinaryOptionsToolsV2.pocketoption.tools.login import _login_captcha_solver
        with patch("BinaryOptionsToolsV2.pocketoption.tools.login._solve_via_capsolver", return_value="mock_token"):
            with pytest.raises(LoginError, match="cookie was not set"):
                _login_captcha_solver("u@e.com", "p", api_key="k", service="capsolver", timeout=10)
    ...
    def test_login_captcha_solver_server_error(self, mock_session_class):
        ...
        from BinaryOptionsToolsV2.pocketoption.tools.login import _login_captcha_solver
        with patch("BinaryOptionsToolsV2.pocketoption.tools.login._solve_via_capsolver", return_value="mock_token"):
            with pytest.raises(LoginError, match="Server rejected login: Invalid email format"):
                _login_captcha_solver("u@e.com", "p", api_key="k", service="capsolver", timeout=10)
    ...
    def test_login_captcha_solver_no_session_cookie(self, mock_session_class):
        ...
        from BinaryOptionsToolsV2.pocketoption.tools.login import _login_captcha_solver
        with patch("BinaryOptionsToolsV2.pocketoption.tools.login._solve_via_capsolver", return_value="mock_token"):
            with pytest.raises(LoginError, match="Invalid credentials: server rejected"):
                _login_captcha_solver("u@e.com", "p", api_key="k", service="capsolver", timeout=10)
    ...
    def test_solve_via_capsolver_creation_error(self, mock_post):
        ...
        from BinaryOptionsToolsV2.pocketoption.tools.login import _solve_via_capsolver
        with pytest.raises(LoginError, match="CapSolver task creation failed: Invalid API key"):
