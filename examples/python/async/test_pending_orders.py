...
async def main(ssid: str):
    async with PocketOptionAsync(ssid) as api:
        ...
        if candles:
            ...
            print(
                f"Could not get candles for {asset}. Using a dummy price for testing."
            )
            current_price = 1.0850
        ...
        pending_price = round(
            current_price - 0.0010, 5
        )  # Far enough to not trigger immediately
        ...
        try:
            ...
            pending_time_str = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.gmtime(pending_timestamp)
            )
            ...
            if tickets:
                print(
                    f"Found {len(tickets)} pending orders. Canceling them in batch..."
                )
