    ...
    if assets:
        ...
        for asset in assets[:5]:
            ...
            try:
                ...
                print(f"  {symbol}: bid={bid:.5f}, ask={ask:.5f}, spread={spread:.5f}, payout={payout*100:.1f}%")
            except Exception as e:
                print(f"  {symbol}: bid={bid:.5f}, ask={ask:.5f}, payout check failed: {e}")
    
    # Find specific asset
    eur_usd = None
    for asset in assets:
        if 'EUR' in asset.get('symbol', '').upper() and 'USD' in asset.get('symbol', '').upper():
            eur_usd = asset
            break
...
async def demo_candle_operations(client: CloseOptionAsync):
    ...
    for period, label in periods:
        try:
            ...
            if candles:
                print(f"    Latest: timestamp={candles[-1].get('timestamp', 'N/A')}, value={candles[-1].get('value', 'N/A')}")
        except Exception as e:
...
async def main():
    ...
    if not ssid:
        ...
        methods = [
            'buy', 'sell', 'check_win', 'balance', 'get_candles',
            'active_assets', 'payout', 'history', 'opened_deals',
            'closed_deals', 'get_server_time', 'send_raw',
            'subscribe_symbol', 'subscribe_raw', 'get_candles_live',
            'raw_handler', 'reconnect', 'shutdown'
        ]
