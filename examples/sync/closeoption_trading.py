#!/usr/bin/env python3
"""
CloseOption Trading Example (Sync)

Demonstrates trading operations with synchronous client:
...
def place_trade(client, asset: str, amount: float, duration: int, direction: str):
    ...
    try:
        if direction.lower() in ('buy', 'call'):
            result = client.buy(asset, amount, duration)
        else:
...
def check_trade(client, order_id: str):
...
def main():
    ...
    try:
        with CloseOption(ssid) as client:
            ...
            #
            # print("\n=== Placing Demo Trades ===")
            # buy_result = place_trade(client, asset, 10.0, 60, 'buy')
            # sell_result = place_trade(client, asset, 10.0, 60, 'sell')
            #
            # if buy_result and 'orderId' in buy_result:
            #     check_trade(client, buy_result['orderId'])
            
            print("\n=== Trading Demo Complete ===")
            print("To place real trades, set CLOSEOPTION_SSID and uncomment trade code above.")
            return 0
